...
private:
    ...
    // 到达容差 (米)
    const float ARRIVAL_TOLERANCE = 0.5;
    ...
    bool checkArrival(int drone_id) {
        if (current_target_index_ >= cmd_num_) return true;
        
        const auto& current_pos = drone_positions_[drone_id].pose.position;
        const auto& target = formation_targets_[drone_id][current_target_index_];
    ...
    void sendTargets() {
        for (int id : drone_ids_) {
            if (current_target_index_ < cmd_num_) {
                const auto& target = formation_targets_[id][current_target_index_];
...
public:
    GroundControl() : drone_num_(0), cmd_num_(0), current_target_index_(0) {
        // 获取参数
